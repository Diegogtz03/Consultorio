...
@app.route('/oauth2callbackcal')
def oauth2callbackcal():
    state = session['state']

    flow = google_auth_oauthlib.flow.Flow.from_client_secrets_file("credentials_calendar.json", scopes=SCOPES_CAL, state=state)
    flow.redirect_uri = url_for('oauth2callbackcal', _external=True)
